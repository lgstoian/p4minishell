...
static const char *usb_entry_type(const struct stat *st)
{
    if (st == NULL) {
        return "UNKNOWN";
    }
...
void usb_status(void)
{
    ...
    if (s_usb_msc.connected) {
        ...
        usb_wide_to_ascii(s_usb_msc.info.iProduct, product, sizeof(product));
        usb_wide_to_ascii(s_usb_msc.info.iSerialNumber, serial, sizeof(serial));
        usb_format_size((uint64_t)s_usb_msc.info.sector_size * (uint64_t)s_usb_msc.info.sector_count,
                        capacity,
                        sizeof(capacity));
...
void usb_msc_ls(const char *path)
{
    ...
    if (S_ISREG(st.st_mode)) {
        ...
        usb_format_size((uint64_t)st.st_size, size_text, sizeof(size_text));
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (S_ISDIR(child_stat.st_mode)) {
            usb_emit_syncf("usb: %s  %s (-)\n", usb_entry_type(&child_stat), entry->d_name);
        } else {
            usb_format_size((uint64_t)child_stat.st_size, size_text, sizeof(size_text));
            usb_emit_syncf("usb: %s %s (%s)\n", usb_entry_type(&child_stat), entry->d_name, size_text);
        }
    }
