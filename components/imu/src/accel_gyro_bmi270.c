/*
 * Vendored from M5Stack M5Tab5-UserDemo (sensor_bmi270, MIT); the Bosch
 * bmi2/bmi270 sources it wraps are BSD-3-Clause. Local change: the I2C device
 * add no longer aborts via ESP_ERROR_CHECK so a missing IMU degrades cleanly.
...
esp_err_t accel_gyro_bmi270_init(i2c_master_bus_handle_t bus_handle)
{
    ...
    if (i2c_master_bus_add_device(bus_handle, &dev_cfg, &i2c_dev_handle_bmi270) != ESP_OK ||
        i2c_dev_handle_bmi270 == NULL) {
        ESP_LOGE(TAG, "failed to add BMI270 on I2C");
        return ESP_FAIL;
    }
