...
/* This vendored BSP is driven by board_config.h rather than a Kconfig, so it
 * can coexist with the EV-board managed BSP in the same managed_components
 * tree without duplicate CONFIG_BSP_* symbols. */
#define BOARD_BSP_ERROR_CHECK          1
#define BOARD_BSP_LCD_USE_DMA2D        1
#define BOARD_BSP_LCD_DRAW_BUF_HEIGHT  50
/**************************************************************************************************
 *  BSP Board Name
 **************************************************************************************************/
 ...
 *
 **************************************************************************************************/
#define BSP_I2C_NUM     BOARD_CFG_I2C_PORT
 ...
 * \endcode
 **************************************************************************************************/
#define BSP_SPIFFS_MOUNT_POINT      BOARD_CFG_SPIFFS_MOUNT_POINT
 ...
 *
 **************************************************************************************************/
#define BSP_SD_MOUNT_POINT      BOARD_CFG_SD_MOUNT_POINT
 ...
 *        PI4IOE5V6408 (P4) three times.
 *
 * The Tab5 PMIC cuts the main rails when P4 of IO expander 0x44 is pulsed; the
 * pulse is repeated because a single edge can be missed. Restores the M5Stack
 * BSP behaviour (this trimmed tree dropped bsp_power.c).
 */
void bsp_generate_poweroff_signal(void);

/**
 * @brief Enable/disable battery charging (PI4IOE5V6408 0x44 P7) and QuickCharge
 *        (P5, active low).
 *
 * Charging is off after reset on the Tab5, so the firmware must enable it to
 * charge the pack. This only gates the charge path; it never sets charge
 * voltage/current.
 */
...
/* The BMI270 IMU / sensor_hub API is intentionally not exposed: this port does
 * not vendor bsp_sensors.c (no IMU consumer). */
