...
static bsp_tab5_board_version_t bsp_get_board_version(void)
{
    ...
    /* Board-config override for field units whose TDDI/GT911 does not answer
     * during the pre-init probe (see BOARD_CFG_LCD_FORCE_VERSION). */
#if defined(BOARD_CFG_LCD_FORCE_VERSION) && (BOARD_CFG_LCD_FORCE_VERSION != 0)
    switch (BOARD_CFG_LCD_FORCE_VERSION) {
    case 1:
        ESP_LOGW(TAG, "Panel revision forced by board_config: ILI9881C + GT911");
        board_ver = BSP_TAB5_BOARD_VERSION_ILI9881C_GT911;
        return board_ver;
    case 2:
        ESP_LOGW(TAG, "Panel revision forced by board_config: ST7123");
        board_ver = BSP_TAB5_BOARD_VERSION_ST7123;
        return board_ver;
    case 3:
        ESP_LOGW(TAG, "Panel revision forced by board_config: ST7121");
    ...
    if (i2c_master_probe(bsp_i2c_get_handle(), ESP_LCD_TOUCH_IO_I2C_ST7123_ADDRESS, 100) == ESP_OK) {
        ...
        /* No known panel/touch responded. Never panic on an unrecognised
         * revision: log the I2C bus so the revision can be identified, and let
         * the caller report the display as unavailable honestly. */
        ESP_LOGE(TAG, "Unsupported board version (no ST712x TDDI at 0x%02X, no GT911 at 0x%02X)",
                 ...
                 ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS_BACKUP);
        {
            ...
            int pos = 0;
            scan[0] = '\0';
            for (uint8_t addr = 0x08; addr < 0x78 && pos < (int)sizeof(scan) - 6; addr++) {
                if (i2c_master_probe(bsp_i2c_get_handle(), addr, 20) == ESP_OK) {
                    pos += snprintf(scan + pos, sizeof(scan) - pos, "%02X ", addr);
                }
            }
            ESP_LOGW(TAG, "I2C bus scan: %s", scan[0] ? scan : "(none)");
        }
        return BSP_TAB5_BOARD_VERSION_UNKNOWN;
    }
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config, bsp_lcd_handles_t *ret_handles)
{
    ...
    /* Detection runs after bsp_enable_dsi_phy_power(): the ST712x TDDI only
     * answers on I2C once its DSI/panel power rail is up. */
    ...
    /* The ST7121 needs a lower DSI lane rate than the default. */
    ...
    const esp_lcd_dpi_panel_config_t dpi_config_ili9881c = {
        ...
        .num_fbs            = BOARD_CFG_LCD_DPI_BUFFER_NUMS,
        .video_timing =
        {
...
#if BOARD_BSP_LCD_USE_DMA2D && (ESP_IDF_VERSION < ESP_IDF_VERSION_VAL(6, 0, 0))
...
static lv_display_t *bsp_display_lcd_init(const bsp_display_cfg_t *cfg)
{
    ...
    /* `bsp_display_new` applies the ST7121 lane-rate override after its (post
     * DSI-PHY-power) revision detection. Do not probe here: the TDDI is not
     * powered yet and a pre-power probe would log a spurious error. */
    ...
    const lvgl_port_display_cfg_t disp_cfg = {
        ...
        .flags = {
            ...
            .full_refresh = true,
#elif BOARD_CFG_LVGL_DIRECT_MODE
            .direct_mode = true,
#endif
        }
    };
...
lv_display_t *bsp_display_start(void)
{
    bsp_display_cfg_t cfg = {
        ...
        .buffer_size = BSP_LCD_H_RES * BOARD_BSP_LCD_DRAW_BUF_HEIGHT,
#if BOARD_CFG_LCD_DRAW_BUFFER_DOUBLE
        .double_buffer = 1,
#else
        .double_buffer = 0,
#endif
