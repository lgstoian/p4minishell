...
{
    /* Prefer SPIRAM so the connect-path RPC control commands (scan responses,
     * events) do not share the internal-RAM heap with LVGL/esp_event (W1).
     * free() in eh_rpc_ctrl_cmd_free() works on heap_caps memory in ESP-IDF. */
    eh_rpc_ctrl_cmd_t *c = (eh_rpc_ctrl_cmd_t *)heap_caps_calloc(
        1, sizeof(*c), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (c == NULL) {
        c = (eh_rpc_ctrl_cmd_t *)calloc(1, sizeof(*c));
    }
    return c;
}
