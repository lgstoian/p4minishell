...
static int sdio_get_tx_buffer_num(uint32_t *tx_num, bool is_lock_needed)
{
	...
	if (ret) {
		/* A stalled slave makes this probe fail on every ~20us credit poll, so
		 * rate-limit the error to the first failure and then at most once a
		 * second. The unstoppable per-poll log flooded the console and cost the
		 * TX task enough time on a busy board to trip the task watchdog. */
...
static void sdio_tx_aggregate_iter(void)
{
	...
	for (;;) {
		...
		for (uint32_t waited_us = 0; waited_us <= (uint32_t)wait_ms * 1000u;
			...
			/* Yield a full tick, not a sub-tick busy-wait: esp_rom_delay_us
			 * does not schedule, so a stalled slave kept this task spinning on
			 * the core and starved the idle task into the task watchdog. A tick
			 * still gives 12/200 chances inside the bulk/ctrl wait window. */
