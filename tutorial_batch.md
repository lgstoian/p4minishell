...
For the exhaustive language reference see [`batch.md`](batch.md) (canonical);
the batch sections of [`command.md`](command.md) summarize the same verbs.
...
```bat
@echo off
if "%~1"=="" (echo usage: GREET name & exit /b 2)
set NAME=%~1
echo Hello, %NAME%!
```
...
- `for /f "eol=; skip=1 delims=, tokens=1,2" %%a in (data.csv) do echo %%a=%%b`
  iterates file lines.
- `for /L %%i in (1,1,5) do echo %%i` counts (negative step counts down).
- `while n LSS 3 do set /a n=n+1` loops while the `set /a` expression is
  nonzero (bare names or `%%n%%` stay live per pass — see
  [`batch.md`](batch.md) §10.5).
- `for /A %%k in (SPR) do echo %%k=%%l` iterates an array;
  `for /D %%v in (P*)` matches directories; `for /R %%v in (*.TXT)` walks
  the tree; `switch %MODE% setup:cfg diag:dg /d:help` string-dispatches;
  `setlocal enabledelayedexpansion` enables live `!VAR!` reads.
- `set SPR[0]=10` + `%SPR[0]%` indexed arrays; `%S:~1,3%` substrings and
  `%S:old=new%` replacement. The full language lives in
  [`batch.md`](batch.md).
...
1. Add an `APPINFO` file (`sd:/APPS/<APP>.APPINFO`, INI format):
   ...
   (Hybrid bundles set `type=hybrid` plus `abi=`/`arch=`/`entry=`; native
   bundles set `type=native`. See [`ABI.md`](ABI.md) and
