...
- Registration: call `whoami_register()` once beside
  `native_apps_register()` (see `main/main.c`). It lands in the same
  `app_register` table as `hello`, so dispatch, ERRORLEVEL, and `>`/`>>`
  redirection behave identically.
...
command-sized stack locals), bounded input, real ERRORLEVEL, no direct LVGL
