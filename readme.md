...
**Version:** 0.36.0 | **Target:** ESP32-P4 + ESP32-C6 | **Display:** JD9165 1024x600 MIPI-DSI
...
components/command/         Command module (parser, dispatcher, output-redirection parsing, worker task + background-job pool, and the split `*_commands.c` verb files: tui/gfx/serial/font/md/json/asset/pkg/db/alarm/config/power/periph/audio)
...
| `components/gfx/` | Pure RGB565 raster surface (alloc/pixel/line/rect/circle/hline/vline/triangle/ellipse/polygon/flood-fill/text/get, `gfx_font.c` 8x8 ASCII font), BMP header parse + decode, and 565→888 row conversion; the display glue is `gfx_commands.c` |
| `components/db/` | The Palm-OS-style SD record store (`sd:/DBS/<name>.DB`): records, categories, secret/redacted payloads, soft-delete + purge |
...
- **GFX pixel canvas + sprites**: `gfx init/clear/pixel/line/rect/circle/show` draw to a PSRAM RGB565 canvas (max 320x240) shown in the transcript region; `gfx load <slot> <path>` ingests a 24-bit BI_RGB BMP into one of 8 sprite slots (max 64x64), `gfx blit` stamps it (clipped, optional transparency), `gfx save` writes the canvas back as BMP, and `gfx free`/`gfx slots` manage the bank. Exclusive with TUI mode and refused in background jobs. Reference: `apps/gfxdemo/BOUNCE.BAT`.
...
| **Files & assets** | `open <file>` (dispatch by type), `view`, `hexview`, `crc32 <path>`, `asset check\|list <app>`, `pkg list\|info\|verify\|check\|install\|remove` |
| **Background jobs** | `start <command> [args]`, `taskkill <job>` |
| **Data / state** | `db …`, `ini get\|set\|del\|list\|load\|save`, `appconfig <app> …`, `temp new\|clean`, `json validate\|pretty`, `markdown <file>\|-e <text>\|on\|off` |
