...
- `components/led/` is a LEAF status/notification engine (WS2812 on
  `P4_CONFIG_LED_GPIO` over RMT). `led_notify()` MUST keep the persistent
  `s_status_frame` current for Wi-Fi status events (`WIFI_CONNECTING` /
  `CONNECTED` / `DISCONNECTED`) even while the auto layer is off — otherwise
  `rgb auto on` shows a stale colour (the event only flashed transiently). In
  manual mode status events still flash transiently. The module never queries
  other subsystems; consumers push `led_notify()` events. `tools/led_watch.py`
  is the webcam verifier (locate + exposure-invariant colour classification).
...
the raster core + 8x8 font are `components/gfx/`
  - `gfx show` MUST `lvgl_port_task_wake(LVGL_PORT_EVENT_DISPLAY, NULL)` after
    ...
    (500 ms) between timers, so an invalidation alone is not redrawn promptly and
    a frame loop presents only a few fps (jerky). Never call blocking I/O
    (`screenshot` re-renders the whole screen + writes a multi-MB BMP) inside a
...
- Foreground break (Ctrl+C / input-row Stop) is cooperative: `shell_request_abort()` sets the
  flag; the batch line loop, `for` bodies, and `delay` chunks poll it and unwind the whole
  script with `^C`. `delay` MUST NOT consume the request (`shell_clear_abort()`) — doing so
  stopped only the delay and let the script run on, so the Stop button looked dead. An aborted
  command also leaves the gfx canvas / TUI / app mode open (hiding the prompt), so the worker
  MUST tear those down after a break (`command_cleanup_foreground_surfaces`: `gfx_force_close()`,
  `tui_deinit()`, `shell_app_mode_exit()`).
