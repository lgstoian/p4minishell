...
- `CONFIG_ESP_HOSTED_HOST_SDIO_CLK_KHZ=40000` is the adopted hosted SDIO clock (v0.38.0). The
  ...
  Wi-Fi+SD soak, 0 stalls + full suite). To compare throughput, use `wifi throughput
  tx|rx [port=N] [mb=N] [udp]` (device) with `tools/wifi_bench.py` (host) — both must be on the same
  subnet. `tools/regression.py` runs the whole host suite in one command.
...
### TUI Rules (v0.38.0 - 80x25 grid in the live transcript rect, `components/tui/`, font 384 glyphs, `draw`/`gfx`, fullscreen, screenshot)
