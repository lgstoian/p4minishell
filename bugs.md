...
- **O3. MOSTLY FIXED — per-append transcript span rebuilds went O(buffer); deferred batching landed, 30-min probe exonerates the worker (2026-09-08 session).** Root cause as before (`shell_transcript_append_internal()` repainted LVGL spans O(buffer) per append; drain bench 0.9 KB/s full-buffer). Fix shipped: `P4_CONFIG_TRANSCRIPT_FLUSH_MS=200` deferral (`shell_transcript_defer_begin/end`, `shell_transcript_flush_now`, `shell_transcript_maybe_update_label`), PORT-outer/BUF-inner buffer mutex, segment batching in `command.c` — drain now 19–29 KB/s both runs, deep 8/8, modal/edit/db green. Follow-up 30-min `tools/stall_catch.py` run (740 probes, echo-vs-output timing, glue-tolerant matcher): zero queue-fulls, zero heap warns, db/sd typically 0.0–0.2 s, three output-absence episodes — two traced to bad expects (`db.databases`/`fs_total` vary by output shape), one genuine single-output loss (~1/740, TX-heavy window, self-healed, mechanism unproven; UART mirror is blocking `printf`, so device-side drop is unlikely). Residual: rare single-output loss under TX pressure can still flip one barrier check per long session; host mitigations stand (output-anchored barriers, glue-tolerant `worker_line` in deep/db/alarm drivers, prompt-glue documented as normal serial behavior — trailing prompt has no newline by design). The old-queue-full storm signature (db5/db6, pre-fix firmware) has not recurred post-fix.
- **O4. NOTED — host tooling reboots the board (2026-09-07 session).** Opening COMx with pyserial defaults asserts DTR and the P4 resets (proven: uptime 2m53s → 0m1s across a bare reopen). Every probe script used to produce suspect evidence; `shell_session.py` already cleared DTR/RTS (safe), and `deep_test`/`db_test`/`alarm_test`/`run_companion`/`push_sd` now do too, plus an SD-mount gate in `hw_reset` against N1 wedged boots (~1/15) and quiesce/retry in `push_sd.py` (boot logs used to corrupt the ACK protocol).
- **O5. RECOVERED — silent-boot wedge needed a power cycle (2026-09-07 session).** After hours of unclean DTR resets the board stopped past ROM `entry` on both firmware images (black display, USB alive, esptool functional) — suspected wedged external I2C/PSRAM slave holding a bus; chip resets don't clear powered slaves. Physical unplug/replug recovered fully. If seen again, suspect display/touch I2C or PSRAM init hangs before console bring-up.
- **O6. NOTED — synchronous SD ops have a huge latency tail (shared-bus contention); hot-loop file deletes removed from reference apps.** `del /p _mood.txt` per dashboard sample wedged the worker >60 s (console echoes kept answering, no queue-fulls at slow send rates, `calc` instant on recovery probe) while the identical delete is instant most of the time; same signature as the 30–35 s smoke stalls and plausibly the old pre-fix db7 queue-full storm under sustained SD load. By elimination the stall is inside the synchronous FATFS/SDMMC path (only SD op in the flow; everything else is CPU), consistent with the known shared-SDMMC N1 contention family (O4 gate covers the boot wedge). No firmware change: apps avoid hot-loop deletes (`>` overwrites; `apps/mood/MOOD.BAT`, `apps/notes/NOTES.BAT` carry O6 rems) and host drivers keep full caps + output-anchored barriers so a slow op costs time, never correctness.
