...
- **O3. OPEN — transient worker stalls + queue-full storms under session load (2026-09-07 session).** After long sessions (deep/db/alarm/sweeps back-to-back), commands intermittently take ~40 s or queue up with `shell: command queue full, command dropped`, then recover spontaneously — no panic, no reboot, memory healthy (heap 93%, internal ~58 KB both states). Big-output drain measured ~1 KB/s device-paced (a 1.3 MB `type` streams for 20+ min), suggesting per-append full transcript span rebuilds on the 64 KB buffer go O(buffer) and trim cycles modulate it. Hypotheses (unproven): LVGL-lock contention on span rebuilds, USB TX backpressure (`printf` to a 4 KB ring blocks while the host sleeps through driver settles/esptool phases), SDMMC SD+C6 bus contention (N1 family). Mitigations landed: barrier-synced drivers (`echo DONE-n` FIFO framing in `db_test`/`alarm_test`) attribute slow commands correctly instead of cascade-misattributing; needs firmware-side tracing (lock-order audit, shell_cmd WDT, span-append profiling) as follow-up. Campaign verdicts stand (every suite green at least once; deep 8/8).
...
| Open | 3 (O3 transient worker stalls + queue-full storms, ~1 KB/s drain; O4 host DTR-open reboots; O5 silent-boot wedge) | O3 OPEN — needs firmware tracing; O4/O5 mitigated, see OPEN section. O1/O2 fixed in 0.35.1. |
