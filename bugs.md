# P4MiniShell — Bug Report & Test Campaign (v0.38.5, suite 300/0/2)

Date: 2026-08-24
Hardware: ESP32-P4 (rev 1.0) on COM11, ESP32-C6 co-processor, JD9165 display, SD card present
Firmware: **v0.38.5**. Current config baseline: transcript 65536 (`p4minishell_config.h:93`), async 512 (`:146`), internal trim 4096 (`:120`), SD DMA 4096 (`:621`), command worker stack 32768 (`:1467`), batch-file RAM cap 131072 (`:797`), TUI 80x25 (`:325/:328`). Suite: **300/0/2**.
...
- **O11. ✅ FIXED — BOUNCE.BAT animation was sluggish/jerky and froze mid-run (2026-09-15 session).** Two independent causes. (1) `gfx show` only called `lv_obj_invalidate()`; the ESP LVGL port task sleeps up to `task_max_sleep_ms` (500 ms) between timers, so the invalidated canvas waited for the next unrelated timer — a 20 fps loop presented only ~4–5 frames/s (the ball jumped ~3 frames at a time). Fix: `gfx show` now calls `lvgl_port_task_wake(LVGL_PORT_EVENT_DISPLAY, NULL)` so the canvas presents each frame. (2) The loop ran `if %n% EQU 30 screenshot BOUNCE.BMP`; `screenshot` re-renders the whole 1024x600 screen (`lv_snapshot_take`) and writes a 1.8 MB BMP, freezing the animation ~3 s. Fix: the capture runs once after the loop, and the run is 240 frames at ~30 fps. Webcam ball-tracking (`tools/display_glitch_watch.py`-style, mean blue/white centroid): **35% → 85%** of camera frames show the ball moving, max freeze 2.5 s → 0.1 s. `bounce_test.py` (pass1+mid-flight pass2) and `bounce_twice.py` pass; unit 300/0/2; `pkg info BOUNCE` verifies the repushed manifest.

- **O12. ✅ FIXED — batch/canvas apps slowed down over time and the Stop button did nothing (2026-09-15 session).** Two bugs. (1) Every batch line is wrapped in `shell_transcript_defer_begin/end()`, and the end flushed the transcript label, which copies the whole ANSI transcript (up to 64 KB) into the staging buffer and rebuilds spans. While a gfx canvas/TUI/app hides the transcript these spans are invisible, so the work was wasted and grew with the transcript — BOUNCE's fps decayed 26 → 15 over ~35 s (measured), then plateaued; the per-line copy also churned internal RAM. Fix: `shell_transcript_update_label()` skips staging + span rebuild while `windows_transcript_is_hidden()`, and the header poll repaints once when the transcript reappears (steady ~27 fps now). (2) The Stop button / Ctrl+C did nothing because `delay` consumed the foreground-break flag (`shell_clear_abort()`) to stop only the delay, so the batch line loop never saw it; and aborting left the gfx canvas open, hiding the prompt. Fix: `delay` marks the break and leaves the request set (the line loop unwinds with `^C`); after an aborted command the worker tears down foreground surfaces (`gfx_force_close()`, `tui_deinit()`, `shell_app_mode_exit()`). Verified: simulated Stop aborts a long batch mid-run, `gfx status` reports no canvas, prompt returns; `boot_regression` 10/10; unit 300/0/2.

- **O13. ✅ FIXED — status LED kept a stale colour after `rgb auto on` (2026-09-15 session).** `led_notify()` updated the persistent status frame only when the auto layer was already on; a Wi-Fi status event arriving in manual mode (e.g. `WIFI_CONNECTED`) was shown as a 1.5 s transient and left the status frame stale (amber `#FF9900` from boot). Enabling auto then displayed amber while the board was connected. Fix: status events (`WIFI_CONNECTING`/`CONNECTED`/`DISCONNECTED`) always refresh the status frame, and additionally flash transiently while auto is off. Verified by the new webcam tester `tools/led_watch.py` (LED located + colour-classified): solids red/green/blue/amber/cyan/white/off, effects rainbow/breath/pulse/blink, auto on/off, `httpd start` blue transient, and `rgb auto on` while connected -> green — 18/18.
