...
/**
 * Period of the transcript apply pump on the LVGL task (milliseconds). The
 * command worker stages text and only wakes the port task; the apply timer
 * consumes it within one tick (bounds worst-case visible staleness of the
 * transcript to this period plus one render). Small enough to feel instant,
 * large enough that a burst of lines coalesces into few applies. Replaces
 * the per-append lv_async_call that serialized the worker behind the LVGL
 * port lock (bugs.md F26).
 */
#define P4_CONFIG_TRANSCRIPT_APPLY_TICK_MS     10
...
/** Maximum sprite stamps one `gfx blitmany` command draws (pairs of X Y).
 * Bounds one command's argv and canvas work so a batch game loop stays
 * predictable; stamp more by issuing the verb again. */
#define P4_CONFIG_GFX_BLITMANY_MAX              64
...
/**
 * Pack-presence confirmation for fuel-gauge boards (M5Stack Tab5, bugs.md F24).
 *
 * With no pack attached the INA226 bus node floats to the charger/system rail,
 * so samples alternate between ~8.40 V (>= BOARD_CFG_BATTERY_FULL_MV) and a low
 * floating value, both with ~0 shunt current. A reading clearly inside the pack
 * range is trusted immediately; a reading inside the ambiguous "rail band"
 * (>= FULL_MV - P4_CONFIG_BATTERY_RAIL_BAND_TOL_MV) is only accepted once the
 * last P4_CONFIG_BATTERY_PRESENT_STABLE_SAMPLES readings are all plausible and
 * within P4_CONFIG_BATTERY_PRESENT_MAX_SWING_MV of each other. A floating node
 * fails the swing test; a real pack is stable.
 */
#define P4_CONFIG_BATTERY_PRESENT_STABLE_SAMPLES  3

/** Maximum peak-to-peak swing (mV) across the presence window that still
 *  counts as a stable pack voltage. A floating rail swings volts. */
#define P4_CONFIG_BATTERY_PRESENT_MAX_SWING_MV    800

/** Width (mV) of the ambiguous rail band below BOARD_CFG_BATTERY_FULL_MV.
 *  Readings at or above `FULL_MV - this` need the stability window. */
#define P4_CONFIG_BATTERY_RAIL_BAND_TOL_MV        200
...
/* ========================================================================
 * DECLARATIVE SCREEN FLOWS (`screen flow <file.flow>`)
 * ========================================================================
 * A flow is a flat `KEY=VALUE` navigation graph whose nodes point at `.FRM`
 * screens. The table lives on the heap (the driver runs on the recursive
 * command path), so these caps bound one flow file, not the stack. */

/** Maximum distinct steps (screen nodes) one flow file may declare. */
#define P4_CONFIG_SCREEN_FLOW_STEPS          16

/** Maximum routing rules across all steps of one flow (file order kept). */
#define P4_CONFIG_SCREEN_FLOW_RULES          64

/** Maximum step transitions one `screen flow` run may take before it stops
 *  with ERRORLEVEL 1 (bounds a mis-wired loop). */
#define P4_CONFIG_SCREEN_FLOW_ITER_MAX       256

/** Maximum length of a step id / step name (bytes, including NUL). */
#define P4_CONFIG_SCREEN_FLOW_ID_BYTES       16
...
/** Native-app ABI tag (see ABI.md). `pkg install` warns
 *  when a native/hybrid bundle's `abi=` differs; execution refuses. */
#define P4_CONFIG_NATIVE_ABI                 "applib-1"

/** Manifest trust policy: 1 refuses unsigned bundles at `pkg install`
 *  (warns otherwise); 0 warns on unsigned but still installs. A present but
 *  BAD signature always refuses, whatever this says. `pkg install /signed`
 *  enforces per invocation. See ABI.md. */
#define P4_CONFIG_PKG_REQUIRE_SIGN           0
...
/* ========================================================================
 * GLOBAL FIND: TEXT-FILE SCOPE (`gfind <text> /files`)
 * ========================================================================
 * `gfind` searches the structured stores by default; `/files` adds an opt-in
 * recursive scan of text files under a root. The scan reuses the shared
 * storage walk/scan core (the same one `findstr /S` uses); these caps bound
 * one invocation so a huge card cannot flood the transcript or stall. */

/** Maximum files `gfind /files` opens in one run. */
#define P4_CONFIG_GFIND_MAX_FILES            256

/** Files larger than this are skipped by `gfind /files` (bytes). */
#define P4_CONFIG_GFIND_MAX_FILE_BYTES       131072

/** Maximum matching lines `gfind /files` prints before it truncates. */
#define P4_CONFIG_GFIND_MAX_MATCHES          256

/** Default for the file scope when `/files` is not on the command line
 *  (0 = stores only, 1 = search files everywhere by default). */
#define P4_CONFIG_GFIND_SEARCH_FILES_DEFAULT 0

/** Directory names the file scope never descends into (comma-separated,
 *  case-insensitive): the structured stores are covered by their own scopes,
 *  so scanning their raw files would double-report. */
#define P4_CONFIG_GFIND_SKIP_DIRS            "DBS,ALARMS"
...
/* ========================================================================
 * `for /L` NUMERIC LOOPS and `while` CONDITION LOOPS
 * ========================================================================
 * `for /L %%v in (start,step,end) do cmd` counts like cmd.exe (inclusive of
 * both ends; a negative step counts down). `while <expr> do cmd` re-runs its
 * body while the `set /a` expression is nonzero. Both bounds stop a runaway
 * loop from wedging the command worker; a foreground break (^C / Stop)
 * unwinds either loop the same way it unwinds `for`.
 */

/** Maximum iterations one `for /L` loop runs (0 = the loop body never runs twice without bound). */
#define P4_CONFIG_FORL_ITER_MAX              100000

/** Maximum iterations one `while` loop runs before it stops with ERRORLEVEL 1. */
#define P4_CONFIG_WHILE_ITER_MAX             100000
 ...
 * command / menu. Sized past the bundled app count (plus root-level utility
 * scripts, which share the table) so late discovery entries are not
 * silently dropped. The table is heap-allocated per `launch` call. */
#define P4_CONFIG_LAUNCH_MAX                    40
...
/** Bytes encrypted per chunk during a `crypt` file run. Kept small so the
 *  esp-aes hardware path needs only a tiny DMA descriptor array; a larger
 *  chunk fails on a board with a fragmented DMA-capable heap (M5Stack Tab5
 *  after a busy session). See bugs.md F23. */
#define P4_CONFIG_CRYPT_CHUNK_BYTES          512
...
/** Minimum contiguous DMA-capable bytes required before `crypt` attempts the
 *  esp-aes hardware path. Below this the software AES-256-GCM fallback runs
 *  straight away; a hardware run that still fails on crypto is also retried
 *  once through software. Sized for two cache-aligned chunk buffers plus the
 *  tiny GDMA descriptor array with margin (see bugs.md F23). */
#define P4_CONFIG_CRYPT_DMA_MIN_BYTES        4096
